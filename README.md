...
coara 的内核只有一个常驻进程，但**一切皆工作空间**——一个项目目录、一个对话空间、一个 agentic 软件（工作流画布），在 coara 里都是同一种东西：工作空间。

内核围绕工作空间组织一切：记录、消息、审批、展示、工具、技能，全部挂在工作空间上运转，而不是游离在外。多路显示端——**终端 CLI**、**浏览器 Web UI**、**手机 App**——连接同一个内核，进入同一份工作空间。

## 一切皆工作空间

空间 = 数据 + 投影 + 写入契约。界面是数据在不同视图下的投影，差异只在数据本身。一个普通对话目录和一个复杂 agentic 软件是同一模型，不分两种东西。
