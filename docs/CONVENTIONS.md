...
### 新代码门禁（2026-09-16 起，存量只减不增）

- **禁止新增 `getattr(self, "_…", default)` 防御式属性访问**：实例属性一律在 `__init__` 声明（动态回填的声明为 `None`）；`None` 语义本身确需防御的，保留并加注释说明原因。存量基线 60 处，逐批消除，只减不增
- **禁止新增裸 `except Exception: pass`**：预期异常用具体类型（`OSError` 等）；确实兜底时至少 `logger.debug(..., exc_info=True)`；用户可感知的数据丢失路径至少 `warning`；有意静默回落（终端能力差异、fail-open 有注释处）须保留注释说明意图。存量基线 62 处（宽口径 50 + OSError 系 12），逐批消除，只减不增
