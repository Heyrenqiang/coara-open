...
**LLM 回合内** `ws(action="switch")`：源会话以 `CoaraRunCancelledError` 结束；`strip_ws_switch_tail` 从 `ws(switch)` 上溯到触发切换的用户输入并删掉该段尾部，strip 后若本回合已有工具落盘则追加一条磁盘副作用注记；目标会话历史不动。离开空间的其余历史留在自己的 `WorkspaceSession`，切回即恢复。
