...
class RootCoara(CoaraBase):
    ...
    def __init__(
        ...
        self.telemetry_service = None  # root_lifecycle 装配（TelemetryService）
        self.matrix_host = None  # root_lifecycle 装配（GoMatrixHost，gomatrix 托管）
        self.records_store = None  # set in root_lifecycle（统一 records/{agent,user}）
        ...
        # 工作空间概况（ws.md）维护：按空间记录已维护的 activity epoch，防重复触发。
        self._janitor_activity_at: dict[str, float] = {}
        # 维护进行中：workspace_id -> (task_id, activity_epoch)。
        self._janitor_pending: dict[str, tuple[str, float]] = {}
        # 启动补扫只跑一次。
        self._janitor_startup_scan_done: bool = False
    ...
    async def set_view_workspace(
        ...
        *,
        ...
        if end_key == "cli" and not previous_workspace_id:
            previous_workspace_id = str(getattr(self, "_foreground_session_id", None) or "")
    ...
    def switch_llm_global(
        ...
        manager = self.workspace_manager
        entry = manager.active_entry if manager is not None else None
    ...
    def _bump_janitor_activity_disk(self, workspace_id: str, activity_at: float) -> None:
        """顺延磁盘已维护 epoch（与内存 _janitor_activity_at 对齐）。"""
        session = self._sessions.get(workspace_id)
    ...
    async def _janitor_startup_scan(self) -> None:
        ...
        """
        if self._janitor_startup_scan_done:
            return
        self._janitor_startup_scan_done = True
        wm = self.workspace_manager
        if wm is None:
            return
        ...
        for entry in entries:
            ...
            if not has_real_conversation_in_history(history, session_id=last_sid):
                continue
            if self._janitor_activity_at.get(entry.id) == last_updated:
                continue
            ...
            if load_janitor_activity(str(ws_dir), str(wm.coara_home)) == last_updated:
                ...
                self._janitor_activity_at[entry.id] = last_updated
                continue
    ...
    async def _updates_sweep_tick(self) -> None:
        ...
        push = getattr(self, "_push_workspace_updates_state_to_matrix", None)
        if push is not None:
            try:
                await push()
            except Exception as exc:
    ...
    async def _janitor_finalize_pending(self) -> None:
        """Finish maintenance for tasks no longer running: race check + renew.

        在飞判定查流程自管 flight 表（机制化后 janitor 不经 BackgroundAgentManager）。
        ``task_id == "maintained"``：压缩/先前维护已完成，只待空闲续会话，无在飞任务。
        """
        from src.coara.workspace_protocol import _janitor_flights

        for ws_id, (task_id, epoch) in list(self._janitor_pending.items()):
            if not task_id:
                self._janitor_pending.pop(ws_id, None)
                continue
    ...
    async def _janitor_maybe_renew(self, ws_id: str, epoch: float) -> None:
        ...
        if self._workspace_activity_at.get(ws_id) != epoch:
            logger.info(
        ...
        if self._workspace_has_active_background(ws_id):
            ...
            self._janitor_activity_at.pop(ws_id, None)
            return
        ...
        try:
            ...
            return
        if ws_id != self._foreground_session_id:
            return
    ...
    async def _janitor_scan_expired(self, timeout: float) -> None:
        ...
        for ws_id, session in list(self._sessions.items()):
            activity_at = self._workspace_activity_at.get(ws_id)
            if activity_at is None or not is_session_stale(activity_at):
                continue
            if ws_id in self._janitor_pending:
                continue
            coara = getattr(session, "coara", None)
            ...
            # 原会话。不清 _janitor_activity_at（正常冷却不重复派发），任务收官
            # 后活动时钟随其回合/通知移动，若仍 stale 由 retry 闸门补派放行。
            if self._workspace_has_active_background(ws_id):
                continue
            if self._janitor_activity_at.get(ws_id) == activity_at:
                ...
                if retry_dir and consume_janitor_retry(retry_dir, activity_at):
                    ...
                    self._janitor_pending[ws_id] = ("maintained", activity_at)
                    continue
            workspace_dir = str(getattr(coara, "workspace_dir", "") or "")
            ...
            if coara_home:
                ...
                if load_janitor_activity(workspace_dir, coara_home) == activity_at:
                    self._janitor_activity_at[ws_id] = activity_at
            ...
            if task_id:
                ...
                self._janitor_activity_at[ws_id] = activity_at
                self._janitor_pending[ws_id] = (task_id, activity_at)
