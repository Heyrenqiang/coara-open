...
class SessionHandlers(HandlerMixinBase):
    ...
    def _load_view_messages(
        ...
        list[dict[str, Any]], int, int, dict[str, str], dict[str, list[dict[str, Any]]], dict[str, str], dict[str, int]
    ]:
        """常态 hydrate：从 web 会话视图存储聚合聊天行（web 聊天区唯一数据源）。

        实时显示（TurnStream 帧）与刷新恢复同读这份逐帧落盘的视图文件，
        录像带 projection 不再参与 web 聊天区。返回
        (messages, total, latest_view_seq, subagent_results, subagent_diffs, subagent_briefs, subagent_truncated)。
        ...
        ``subagent_truncated`` = ``{results/diffs/briefs: 被裁条数}``：封顶实际
        裁掉的条数，快照带上它让端上能提示「更早过程已省略 N 项」——封顶是
        边界，不是静默丢失。
        ...
        *target* 由调用方预先解析（快照响应也要这几个字段）；不给时本方法自解。
        ...
        if target is None:
            return [], 0, 0, subagent_results, subagent_diffs, subagent_briefs, subagent_truncated
        ...
        # root 线按端过滤：他端（matrix/cli）的落带帧是线上事实但不投影到本端聊天区
        # ——录制全端统一进带，显示每个端只看自己的（source 取调用方传来的端标识）。
        ...
        return messages, total, latest_seq, subagent_results, subagent_diffs, subagent_briefs, subagent_truncated
    ...
    async def _load_view_snapshot(
        ...
        *,
        ...
        subagent_results, subagent_diffs, subagent_briefs, subagent_truncated, runtime}``。
    ...
    async def _handle_session_model(self, request: web.Request) -> web.Response:
        ...
        if provider:
            ...
            # 有意 getattr 防御：__new__ 测试夹具不跑 __init__，_module_roots 可能不存在
