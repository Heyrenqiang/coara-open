...
import { fetchModelChoices } from '../../../lib/api';
...
export const NodeConfigPanel = memo(function NodeConfigPanel({
  ...
  // 可选模型组合（节点级 LLM 覆盖用）；拉取失败则只给「跟随全局」选项
  const [catalog, setCatalog] = useState<Array<{ provider: string; model: string; label: string }>>([]);
  useEffect(() => {
    let cancelled = false;
    fetchModelChoices()
      .then((s) => {
        if (cancelled) return;
        setCatalog((s.catalog || []).map((c) => ({ provider: c.provider, model: c.model, label: c.label })));
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  return (
    <div className="node-config-panel">
      ...
      <ConfigSection key="llm" title="节点模型" defaultOpen={false}>
        <div className="config-inline-row">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const idx = v.indexOf('/');
              if (idx > 0) {
                updateSelectedNode('provider', v.slice(0, idx));
                updateSelectedNode('model', v.slice(idx + 1));
              } else {
                updateSelectedNode('provider', v || null);
                updateSelectedNode('model', null);
              }
            }}
          >
            <option value="">跟随全局节点模型（默认）</option>
            {catalog.map((c) => (
        ...
        <div className="form-hint">
          默认跟随全局「节点模型」配置；仅当该节点有特殊模型需求（如生成图片/视频）时才改。
        </div>
      </ConfigSection>
