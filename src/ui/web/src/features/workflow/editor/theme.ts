 ...
 *
 * 与全站 theme/tokens.ts 的关系：画布需要一整套 React Flow 专属的
 * 节点/端口/状态色（--wf-*），全站令牌没有这些语义；色值尽量与
 * tokens.ts 同族。门禁（scripts/check-tokens.mjs）对本文件按令牌表放行。
 */
...
const TOKENS = {
  color: {
    ...
    node: {
      bg: '#ffffff',
      /* MiniMap 遮罩（半透明洗色；组件内只经 var(--wf-color-node-minimap-mask) 引用） */
      'minimap-mask': 'rgba(229,231,235,0.5)',
