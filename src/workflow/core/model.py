...
★ 本文件是 coara 自持副本，源自 wdl-engine 的 ``wdl/src/wdl/core/model.py``
（2026-09-10 搬运）。目的是让 coara 的内嵌 flow 能力**不依赖 wdl-engine 是否
安装**——wdl 是独立软件，对 coara 应为可选。

**分叉声明**：本副本自此与 wdl-engine 各自独立演进。wdl 侧的同名文件不会
自动同步到这里。任何图语义（边分类、就绪规则、激活上限）的改动，请明确
判断是否只改本副本、还是两边都要改；漂移哨兵见
``tests/test_workflow/test_core_fork_parity.py``。
