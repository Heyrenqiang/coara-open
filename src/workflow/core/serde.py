...
★ 本文件是 coara 自持副本，源自 wdl-engine 的 ``wdl/src/wdl/core/serde.py``
（2026-09-10 搬运）。分叉声明与漂移哨兵见 ``core/model.py`` 文件头。
...
from src.workflow.core.model import Edge, FlowGraph, Node
...
def _is_default_max_activations(v: int) -> bool:
    from src.workflow.core.model import DEFAULT_MAX_ACTIVATIONS
