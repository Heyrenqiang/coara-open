...
/** 运行状态 → antd Tag 预设色 */
const RUN_STATUS_TAG_COLORS: Record<string, string> = {
  starting: 'processing',
  running: 'processing',
  waiting: 'warning',
  completed: 'success',
...
export interface WorkflowEditorProps {
  /** 当前打开的 WDL 文件名（相对根目录），编辑器直接读写该文件。 */
  draftId: string;
  /** 当前实例的节点实时状态（WS 引擎事件投影；画布徽章 + 运转侧栏）。 */
  liveNodes?: Record<string, FlowLiveNode> | null;
  /** 运行按钮触发后的实例 id 上报（工作台据此订阅该实例的 WS 事件）。 */
  onRunInstance?: (instanceId: string) => void;
}
...
export function WorkflowEditor({
  ...
  /* ---- 自动保存调度器：dirty 比对 + 保存串行化（台账 #216），每个草稿一个实例 ---- */
  ...
  useEffect(() => {
    ...
    const scheduler = new AutosaveScheduler<KernelDocument>({
      save: async (doc) => {
        ...
        // 切换草稿后旧调度器的迟到回调只完成网络请求，不碰新草稿的 UI state（台账 #341）；
  ...
  /* ---- coara 会话编排写穿与 FlowRoot live WDL 灌入均已随迁移移除：
   * WDL 软件无会话概念，画布唯一数据源就是文件本身 ---- */
  ...
  /* ---- Run-state → node status mapping ---- */
  const nodeStatusFromRunState = useCallback(
    (stepId: string, runState: WorkflowRunView): string => {
      if (!runState || !stepId) return 'pending';
      const completed = new Set(
        ((runState.completed_steps as string[]) || []).map(String),
      );
      const current = runState.current_step_id ? String(runState.current_step_id) : '';
      if (current === stepId) {
        if (runState.status === 'failed') return 'error';
        if (runState.status === 'running') return 'running';
        if (runState.status === 'waiting') return 'waiting';
        return 'success';
      }
      if (completed.has(stepId)) return 'success';
      return 'pending';
    },
    [],
  );
  ...
  const applyRunStateToNodes = useCallback(
    (runState: WorkflowRunView) => {
      setNodes((nds) =>
        nds.map((n) => {
          if (!n.data?.stepId) return n;
          const status = nodeStatusFromRunState(n.data.stepId, runState);
          return { ...n, data: { ...n.data, status } };
        }),
      );
    },
    [nodeStatusFromRunState, setNodes],
  );
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        const view = await getWorkflowInstance(runInstanceId);
        if (cancelled) return;
        applyRunStateToNodes(view);
        setRunStatus(String(view.status || ''));
        setRunError(String(view.error || ''));
        const prog = view.progress as RunProgress | undefined;
        setRunProgress(prog || null);
        if (
        ...
        console.warn('poll run state failed', err);
  ...
  }, [runInstanceId, applyRunStateToNodes]);

  /* ---- FlowRoot live node status → canvas badges（引擎 Run 优先） ---- */
  const engineRunActive =
  ...
  }, [liveNodes, engineRunActive, setNodes]);
  ...
  const liveProgress = useMemo(() => {
    ...
    return { pending, running, done, failed, total: vals.length };
  }, [liveNodes]);

  async function handleRunWorkflow() {
    if (!draftId) {
      setRunError('缺少 draftId');
      return;
    }
    try {
      setRunError('');
      setRunStatus('starting');
      const result = await runWorkflowInstance(draftId);
      setRunInstanceId(result.instance_id);
      onRunInstance?.(result.instance_id);
  ...
  async function handleCancelWorkflow() {
    ...
    try {
      setRunError('');
      await cancelWorkflowInstance(runInstanceId);
  ...
  async function handleResume() {
    ...
    try {
      setRunError('');
      setResuming(true);
      await resumeWorkflowInstance(runInstanceId);
    } catch (err) {
      setRunError(err instanceof Error ? err.message : String(err));
    } finally {
      setResuming(false);
  ...
  const onNodeClick = useCallback<NodeMouseHandler<FlowNode>>((_, node) => {
    ...
    setWdlOpen(false);
    setHelpOpen(false);
    // 单击 = 节点配置；节点运转抽屉只经「查看运转」按钮打开，不与配置面板争抢
  ...
  const showStop =
    runStatus === 'running' || runStatus === 'waiting' || runStatus === 'starting';
  ...
  return (
    ...
    >
        ...
        <Space size={8} wrap>
          {liveProgress && (
            <Tag color={liveProgress.running > 0 ? 'processing' : 'default'} style={{ marginInlineEnd: 0 }}>
              {`待${liveProgress.pending}/跑${liveProgress.running}/成${liveProgress.done}/败${liveProgress.failed}`}
            </Tag>
          )}
          {selectedNode && (
            <Button
          ...
          {runStatus && (
            <Tooltip title={runInstanceId ? `实例 ${runInstanceId}` : undefined}>
              <Tag color={RUN_STATUS_TAG_COLORS[runStatus] || 'default'} style={{ marginInlineEnd: 0 }}>
                {`状态: ${RUN_STATUS_LABELS[runStatus] || runStatus}${runProgress ? ` · ${runProgress.completed}/${runProgress.total} (${runProgress.percentage}%)` : ''}`}
              </Tag>
            </Tooltip>
          )}
          {runError && (
          ...
          <ReactFlowProvider>
            ...
            >
              ...
              <MiniMap
                ...
                maskColor="rgba(229,231,235,0.5)"
                pannable
                zoomable
              />
            </ReactFlow>
      ...
      {/* 节点运转面板：右侧面板的一种视图（与配置/WDL/帮助互斥），不占额外位置 */}
      {inspectorOpen && (
        <div
          className="side-panel-floating side-right config-panel-container"
          style={{ width: rightPanelWidth }}
        >
          ...
          <div className="panel-header">
            <span>
              节点运转{selectedNode ? ` · ${String(selectedNode.data?.stepId || selectedNode.id)}` : ''}
            </span>
            <Button
              type="text"
              size="small"
